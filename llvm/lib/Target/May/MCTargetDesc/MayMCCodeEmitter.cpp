...
public:
  MayMCCodeEmitter(const MCInstrInfo &, MCContext &ctx) : Ctx(ctx) {}
...
unsigned
MayMCCodeEmitter::getMachineOpValue(const MCInst &MI, const MCOperand &MO,
                                     SmallVectorImpl<MCFixup> &Fixups,
                                     const MCSubtargetInfo &STI) const {
  if (MO.isReg())
...
unsigned MayMCCodeEmitter::getImm16OpValue(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned
MayMCCodeEmitter::getBranchTarget16OpValue(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(OpNo);
  ...
  Fixups.push_back(
      MCFixup::create(0, MO.getExpr(), MCFixupKind(May::fixup_May_PC16)));
  return 0;
}
